#
#주의!!!! 이 코드를 실행하기 전에 ZipKosdaqCode를 실행해서 코드를 선별하셔야 합니다.
...
excel.Visible = False
wb = excel.Workbooks.Open('C:\\Users\\DG\\PycharmProjects\\Stock\\zipKosdaq.xls')
ws = wb.ActiveSheet

ITEM = 547 #엑셀 맨끝 왼쪽에있는 인덱스 + 1
                    ...
                    if (int(buyFirm_volume) >= 10000):
                        ...
                        if (count == 0):
                            ...
                            if (int(dic[stockName]) < int(buyFirm_volume)):
                                print(nowTime, stockCode, stockName, buyFirm_name, dic[stockName], '->', buyFirm_volume,
                                      '(+', int(buyFirm_volume) - int(dic[stockName]), ')')
                                dic[stockName] = buyFirm_volume
    print("-----------------------------------------")
    count += 1